...
const LoginType = {
  Success: 0,
  PasswordIncorret: 1,
  AlreadyConnected: 2, //If you send this, a text message is sent, but the connection is also disconnected
  CustomMessage: 3, //If you send this, a text message is sent, but the connection is also disconnected
};
...
class LoginHandler extends Handler {
  ...
  handleClientLogin() {
    ...
    AccountModel.findOne({ ID: id }, (err, account) => {
      //TODO: 계정 정지 확인, 계정 접속 상태 확인
      if (!account || account.Password !== pwd) {
        console.error("ID or password incorrect!");
        //this._session.setRandomSessionKey(); 테스트
        this.sendLoginData(0, 0, LoginType.PasswordIncorret);
      } else {
        this._session.initSession(id, () => {
          ...
          account.save();
          this.sendLoginData(account.AccountKey, mac);
        });
      }
    });
  }
  ...
  handleServerList() {
    ...
    AccountModel.findOne({ AccountKey: accountKey }, (err, account) => {
      ...
      this.write(opCode.server.ServerResServerList, ServerList.toBuffer(), () => {
        this.writeRaw(raw_1, () => {
          this.writeRaw(raw_2);
        });
      });
  ...
  handleServerInfo() {
    ...
    this.write(opCode.server.ServerResServerConnection, ServerInfo.toBuffer());
  }
...
*/
    ...
    AuthRes.writeInt8(0).writeInt16LE(0).writeInt8(0).writeInt8(0).writeInt8(0);

    this.write(opCode.login.ServerResLogin, AuthRes.toBuffer());
  }
